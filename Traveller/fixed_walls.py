    ...
    def damage(self,arrows,swords,units):        
        ...
        for k in range(len(swords)):
            if ((swords[k].x1 > self.x and swords[k].x1 < self.x+self.width and swords[k].y1>self.y and swords[k].y1<self.y+self.height) or
                ((swords[k].x1+swords[k].x0)/2 > self.x and (swords[k].x1+swords[k].x0)/2 < self.x+self.width and (swords[k].y1+swords[k].y0)/2>self.y and (swords[k].y1+swords[k].y0)/2<self.y+self.height)) and self != swords[k].owner and swords[k].sharp == 1:
                self.hp -= 30
                swords[k].phi -= 0.1
                swords[k].sharp = 0
        if self.hp<=0:
