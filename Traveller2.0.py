...
class Unit:
    ...
    def stay(self, walls):
        ...
        for i in range(len(walls)):
            if not (self.x + self.width > walls[i].x and self.x < walls[i].x + walls[i].w and self.y < walls[i].y +
                    walls[i].h and self.y + self.height > walls[i].y):
                if self.Vx > 0:
...
finished = False
enemies.append(Enemy(150, 200, 60, 50, 0, 0, 3, 'right', 60, 'sword', None, None, 0))
units.append(Unit(10,500,50,40,0,0,10,'right',75,'sword',None,None,('w','s','a','d')))
